resource "aws_instance" "this" { #second these steps 
  ...
  tags = {  
    Name    = "terraform-remote-state-demo-${var.environment}"
    Project = "roboshop"
  }
}

resource "aws_security_group" "allow_tls"{ #first these steps
  name        ="allow-dynamic-terraform-remote-state-new" # this is for AWS account 
